...
function chooseStartIndex(points, width, height) {
	if (!Array.isArray(points) || points.length < 3) return 0;
	const fillMask = rasterizeRoadMask(points, width, height, {
		halfWidth: 1.22,
		sampleCount: 720,
	}).pixels;
	let best = null;
	...
	for (let index = 0; index < points.length; index += step) {
		...
		const score = (span.width * 4) + curvaturePenalty + topPenalty + edgePenalty;
...
function styleRoadPreview(centerlinePoints, width, height, startPoint) {
	...
	if (startPoint) {
		const span = findHorizontalSpan(fillMask, width, height, Math.round(startPoint[0]), Math.round(startPoint[1]));
		if (span) {
			drawThickLine(pixels, width, height, span.left - 1, span.y, span.right + 1, span.y, 1, 0.1);
		}
	}
...
function buildGeneratedMinimapPreview(track) {
	...
	const canonicalSamples = sampleClosedPath(canonicalPolyline, sampleCount);
	const derivedSamples = sampleClosedPath(derivedCanonicalFit.transformedSourcePoints, sampleCount);
	const alignedDerivedSamples = alignClosedSampleSequence(canonicalSamples, derivedSamples);
	const blendedCanonical = smoothClosedPoints(blendClosedPaths(alignedDerivedSamples, canonicalSamples, 0.42), 3);
	const derivedPreviewFit = fitCanonicalToPreview(
		blendedCanonical,
		previewPoints
	);
	let styledPath = scaleClosedPoints(derivedPreviewFit.transformedCanonicalPoints, 0.84, 0.88);
	styledPath = smoothClosedPoints(styledPath, 1);
	styledPath = expandLowerTail(styledPath, 1.34, 0.66);
	styledPath = fitStyledPathIntoFrame(styledPath, preview.width, preview.height, 2);
	const seamIndex = chooseSeamIndex(styledPath);
	styledPath = rotateClosedPoints(styledPath, seamIndex + 1);
	const startIndex = chooseStartIndex(styledPath, preview.width, preview.height);
	const bounds = getBounds(styledPath);
	const styled = styleRoadPreview(styledPath, preview.width, preview.height, styledPath[startIndex]);
	const selfIntersections = countSelfIntersections(styledPath);
	const lowerTailClearance = computeLowerTailClearance(styledPath, 0.66, 6);
